<template>
  <div @click="visible = !visible" class="exp" :class="{'internship': internship, 'even': even}">
    ...
    <div class="temporal--details">
      From
      <strong>{{experience.from }}</strong> to
      ...
      <b-tabs v-if="internship" pills content-class="mt-3" align="center">
        <b-tab title="Main Activities">
          <div class="details--activities">
            <p v-for="(act, index) in experience.activities" :key="index">
              <triangle variant="warning"></triangle>
              {{act}}
            </p>
          </div>
        </b-tab>
        <b-tab title="Description">
          <div>
            ...
            <p>
              Find more information about {{experience.hostOrganism}} at
              <a :href="`${experience.link}`" target="_blank">{{experience.link}}</a>
            </p>
          </div>
        </b-tab>
      </b-tabs>
